...
namespace MCPForUnity.Editor.Helpers
{
    ...
    public class ToolParams
    {
        ...
        {
            // Try exact match first
            var value = FindToken(key)?.ToString();
            ...
            if (snakeKey != key)
            {
                value = FindToken(snakeKey)?.ToString();
                if (value != null) return value;
            }
            ...
            if (camelKey != key)
            {
                value = FindToken(camelKey)?.ToString();
            }
        ...
        private JToken FindToken(string key)
        {
            ...
            try
            {
                foreach (var property in _params.Properties())
                {
