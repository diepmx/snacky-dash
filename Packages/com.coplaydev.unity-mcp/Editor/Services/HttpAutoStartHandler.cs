...
namespace MCPForUnity.Editor.Services
{
    ...
    {
        private const string SessionInitKey = "HttpAutoStartHandler.SessionInitialized.ProjectForced.v7";
        private static readonly bool ForceProjectHttpBridgeAutoStart = false;
        ...
        static HttpAutoStartHandler()
        {
            ...
            if (!autoStartEnabled && !ForceProjectHttpBridgeAutoStart) return;
        ...
        private static void OnEditorReady()
        {
            try
            {
                ...
                if (ForceProjectHttpBridgeAutoStart)
                {
                ...
                if (!useHttp)
                {
                    McpLog.Warn("[HTTP Auto-Start] HTTP transport is disabled");
                    return;
                }
                ...
                McpLog.Info("[HTTP Auto-Start] Starting project HTTP bridge");
                _ = AutoStartAsync();
            }
            catch (Exception ex)
            {
        ...
        private static async Task WaitForServerAndConnectAsync()
        {
            ...
            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                ...
                if (!EditorPrefs.GetBool(EditorPrefKeys.AutoStartOnLoad, false) && !ForceProjectHttpBridgeAutoStart) return;
