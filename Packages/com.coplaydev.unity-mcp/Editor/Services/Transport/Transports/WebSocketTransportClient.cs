    ...
    public class WebSocketTransportClient : IMcpTransportClient, IDisposable
    {
        ...
        private async Task<bool> EstablishConnectionAsync(CancellationToken token)
        {
            ...
            catch (Exception ex)
            {
                ...
                McpLog.Error($"[WebSocket] Registration exception detail: {ex}");
        ...
        private async Task ReceiveLoopAsync(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    McpLog.Warn($"[WebSocket] Receive exception detail: {ex}");
        ...
        {
            McpLog.Info($"[WebSocket] Received message: {message}");
            string messageType = ExtractJsonString(message, "type") ?? string.Empty;
            ...
            switch (messageType)
            {
                case "welcome":
                    ...
                    break;
                case "registered":
                    await HandleRegisteredAsync(message, token).ConfigureAwait(false);
                    break;
                case "execute":
                    await HandleExecuteAsync(message, token).ConfigureAwait(false);
                    break;
                case "ping":
        ...
        private void ApplyWelcome(string message)
        {
            int? keepAliveSeconds = ExtractJsonInt(message, "keepAliveInterval");
            ...
            int? serverTimeoutSeconds = ExtractJsonInt(message, "serverTimeout");
        ...
        private async Task HandleRegisteredAsync(string message, CancellationToken token)
        {
            string newSessionId = ExtractJsonString(message, "session_id");
        ...
        private async Task HandleExecuteAsync(string message, CancellationToken token)
        {
            string commandId = ExtractJsonString(message, "id");
            string commandName = ExtractJsonString(message, "name");
            string parametersJson = ExtractJsonRawValue(message, "params") ?? "{}";
            int timeoutSeconds = ExtractJsonInt(message, "timeout") ?? (int)DefaultCommandTimeout.TotalSeconds;
            ...
            string commandEnvelope = "{\"type\":" + SerializeJsonString(commandName) + ",\"params\":" + parametersJson + "}";
            ...
            try
            {
                ...
                responseJson = await TransportCommandDispatcher.ExecuteCommandJsonAsync(commandEnvelope, timeoutCts.Token).ConfigureAwait(false);
            }
            catch (OperationCanceledException)
            {
                responseJson = "{\"status\":\"error\",\"error\":" + SerializeJsonString($"Command '{commandName}' timed out after {timeoutSeconds} seconds") + "}";
            }
            catch (Exception ex)
            {
                responseJson = "{\"status\":\"error\",\"error\":" + SerializeJsonString(ex.Message) + "}";
            }

            if (string.IsNullOrWhiteSpace(responseJson))
            {
                responseJson = "{\"status\":\"error\",\"error\":\"Invalid response payload\"}";
            }

            string responsePayload = "{\"type\":\"command_result\",\"id\":"
                                     + SerializeJsonString(commandId)
                                     + ",\"result\":"
                                     + responseJson
                                     + "}";

            await SendJsonStringAsync(responsePayload, token).ConfigureAwait(false);
        }
        ...
        {
            string sessionId = ProjectIdentityUtility.GetOrCreateSessionId();
            string registerJson = "{"
                                  + "\"type\":\"register\","
                                  // Newer servers assign a fresh session_id, but older/strict hubs still
                                  // expect a non-null value during registration.
                                  + "\"session_id\":" + SerializeJsonString(string.IsNullOrEmpty(sessionId) ? Guid.NewGuid().ToString() : sessionId) + ","
                                  + "\"project_name\":" + SerializeJsonString(string.IsNullOrEmpty(_projectName) ? "Unknown Project" : _projectName) + ","
                                  + "\"project_hash\":" + SerializeJsonString(string.IsNullOrEmpty(_projectHash) ? "default" : _projectHash) + ","
                                  + "\"unity_version\":" + SerializeJsonString(string.IsNullOrEmpty(_unityVersion) ? "Unknown" : _unityVersion) + ","
                                  + "\"project_path\":" + SerializeJsonString(_projectPath ?? string.Empty)
                                  + "}";
            McpLog.Info($"[WebSocket] Sending register payload: {registerJson}");
            await SendJsonStringAsync(registerJson, token).ConfigureAwait(false);
        }

        private Task SendPongAsync(CancellationToken token)
        {
            string sessionId = string.IsNullOrEmpty(_sessionId)
                ? ProjectIdentityUtility.GetOrCreateSessionId()
                : _sessionId;
            string payload = "{\"type\":\"pong\",\"session_id\":" + SerializeJsonString(sessionId) + "}";
            return SendJsonStringAsync(payload, token);
        }
        ...
        private async Task SendJsonAsync(JObject payload, CancellationToken token)
        {
            ...
            await SendJsonStringAsync(json, token).ConfigureAwait(false);
        }

        private static string GetJsonString(JObject payload, string name)
        {
            if (payload == null || !payload.TryGetValue(name, out JToken token) || token == null)
            {
                return null;
            }
        ...
        private static int? GetJsonInt(JObject payload, string name)
        {
            if (payload == null || !payload.TryGetValue(name, out JToken token) || token == null)
            {
                return null;
        ...
        private static JObject GetJsonObject(JObject payload, string name)
        {
            if (payload == null || !payload.TryGetValue(name, out JToken token))
            {
                return null;
            }
        ...
        private static string ExtractJsonRawValue(string json, string name)
        {
            ...
            if (first == '{' || first == '[')
            {
                int end = FindJsonContainerEnd(json, index, first == '{' ? '}' : ']');
                return end > index ? json.Substring(index, end - index + 1) : null;
            }

            if (first == '"')
            {
                int end = FindJsonStringEnd(json, index);
                return end > index ? json.Substring(index, end - index + 1) : null;
            }

            int scalarEnd = index;
            while (scalarEnd < json.Length && json[scalarEnd] != ',' && json[scalarEnd] != '}')
            {
                scalarEnd++;
            }
            return json.Substring(index, scalarEnd - index).Trim();
        }

        private static int FindJsonContainerEnd(string json, int start, char closing)
        {
            int depth = 0;
            bool inString = false;
            bool escaped = false;
            ...
            switch (token.Type)
            {
                case JTokenType.Object:
                {
                    ...
                    foreach (JProperty property in ((JObject)token).Properties())
                    {
                        ...
                        builder.Append(':');
                        builder.Append(SerializeJsonToken(property.Value));
                    }
                    builder.Append('}');
                ...
                case JTokenType.Array:
                {
                    ...
                    foreach (JToken item in (JArray)token)
                    {
                        if (!first) builder.Append(',');
                        first = false;
                        builder.Append(SerializeJsonToken(item));
                    }
                    builder.Append(']');
                    return builder.ToString();
                }
                ...
                default:
                    return SerializeJsonString(token.Value<string>() ?? string.Empty);
            }
        }
        ...
        private async Task SendJsonStringAsync(string json, CancellationToken token)
        {
            if (_socket == null)
            {
            ...
            {
                throw new InvalidOperationException("Failed to serialize WebSocket JSON payload.");
            }
