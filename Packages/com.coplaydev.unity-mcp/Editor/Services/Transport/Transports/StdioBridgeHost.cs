...
#pragma warning restore CS0162
                        ...
                        try
                        {
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    response = BuildErrorResponse($"Command processing timed out after {FrameIOTimeoutMs} ms");
                                }
                            }
                            catch (Exception ex)
                            {
                                response = BuildErrorResponse(ex.Message);
                            }
                            ...
                            try
                            {
                                ...
                                {
                                    response = BuildErrorResponse("Empty response payload from Unity command dispatcher");
                                }
        ...
        private static void ProcessCommands()
        {
            ...
            try
            {
                ...
                lock (lockObj)
                {
                    ...
                    if (staleIds != null)
                    {
                        foreach (var sid in staleIds)
                        {
                            ...
                            try { staleCmd.Tcs.TrySetResult(BuildErrorResponse("Command evicted: stuck too long in queue")); } catch { }
                        }
                ...
                foreach (var item in work)
                {
                    ...
                    {
                        tcs.SetResult(BuildErrorResponse("Empty command received"));
                    ...
                    if (commandText == "ping")
                    {
                        tcs.SetResult("{\"status\":\"success\",\"result\":{\"message\":\"pong\"}}");
                        lock (lockObj) { commandQueue.Remove(id); }
                        continue;
                    }
                    ...
                    {
                        tcs.SetResult(BuildErrorResponse(
                            "Invalid JSON format",
        ...
        private static void ExecuteQueuedCommand(string commandId, string payload, TaskCompletionSource<string> completionSource)
        {
            async void Runner()
            {
                ...
                {
                    completionSource.TrySetResult(BuildErrorResponse($"Command processing timed out after {FrameIOTimeoutMs} ms"));
                }
                catch (Exception ex)
                {
                    McpLog.Error($"Error processing command: {ex.Message}\n{ex.StackTrace}");
                    completionSource.TrySetResult(BuildErrorResponse(
                        ex.Message,
                        payload?.Length > 50 ? payload[..50] + "..." : payload
                    ));
                }
                finally
                {
        ...
        public static void WriteHeartbeat(bool reloading, string reason = null)
        {
            try
            {
                ...
                string payload = "{"
                                 ...
                                 + "\"reason\":" + SerializeJsonString(reason ?? (reloading ? "reloading" : "ready")) + ","
                                 + "\"seq\":" + heartbeatSeq + ","
                                 + "\"project_path\":" + SerializeJsonString(Application.dataPath) + ","
                                 + "\"project_name\":" + SerializeJsonString(projectName) + ","
                                 + "\"unity_version\":" + SerializeJsonString(Application.unityVersion) + ","
                                 + "\"last_heartbeat\":" + SerializeJsonString(DateTime.UtcNow.ToString("O")) + ","
                                 + "\"project_scoped_tools\":" + (projectScopedTools ? "true" : "false")
                                 + "}";
        ...
        private static string BuildErrorResponse(string error, string receivedText = null)
        {
            var builder = new StringBuilder();
            builder.Append("{\"status\":\"error\",\"error\":");
            builder.Append(SerializeJsonString(error ?? "Unknown error"));
