...
namespace MCPForUnity.Editor.Tools
{
    ...
    public static class ManageScene
    {
        ...
        private static SceneCommand ToSceneCommand(JObject p)
        {
            ...
            var action = (FindString(p, "action") ?? string.Empty).Trim().ToLowerInvariant();
            if (string.IsNullOrEmpty(action))
            {
                action = ExtractJsonString(TransportCommandDispatcher.CurrentCommandJson, "action") ?? string.Empty;
            }

            return new SceneCommand
            {
                action = action.Trim().ToLowerInvariant(),
                name = FindString(p, "name") ?? string.Empty,
                path = FindString(p, "path") ?? string.Empty,
        ...
        private static string FindString(JObject obj, string key)
        {
